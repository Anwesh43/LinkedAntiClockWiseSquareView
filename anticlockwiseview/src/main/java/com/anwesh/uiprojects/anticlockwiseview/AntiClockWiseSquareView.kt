...
class AntiClockWiseSquareView(ctx : Context) : View(ctx) {
    ...
    data class ACWSNode(var i : Int, val state : State = State()) {

        private var next : ACWSNode? = null
        private var prev : ACWSNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawACWSNode(i, state.scale, paint)
        ...
        fun getNext(dir : Int, cb : () -> Unit) : ACWSNode {
            var curr : ACWSNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class AntiClockWiseSquare(var i : Int) {

        private val root : ACWSNode = ACWSNode(0)
        private var curr : ACWSNode = root
